...
describe('Dashboard Component', () => {
  ...
  test('renders the dashboard and fetches categories', async () => {
    ...
    await waitFor(() => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:5000/categories', { method: 'GET', headers: { authorization: 'fake-token' } });
    });
  });
  ...
  test('handles deleting a category', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:5000/categories/1', { method: 'DELETE', headers: { authorization: 'fake-token' } });
